package com.darran.msga.population;
 ...
 * @param <T>
 */
public class Population<T extends Individual> implements Iterable<T>,Serializable{

	/**
	 * 
	 */
	private static final long serialVersionUID = -1190017808799058506L;
	...
	public void addIndividuals(List<T> individuals) {
		for (T ind : individuals)
			i_individuals.add(ind);
	}
	...
	/**
	 * set the individuals size to i
	 * select best i individuals
	 * @param i
	 */
	public void setIndividualsNum(int i) {
		Collections.sort(i_individuals,Collections.reverseOrder());
		if(i_individuals.size() > i)
			i_individuals = i_individuals.subList(0, i-1);
	}
}
